...
export async function GET(request: NextRequest) {
  try {
    ...
    if (jobTypes && jobTypes.length > 0) {
      where.OR = [
        ...
        ...jobTypes.map((type: string) => ({
          title: { contains: type, mode: Prisma.QueryMode.insensitive },
        })),
      ];
    }
    ...
    if (location) {
      where.restaurant = {
        OR: [
          { address: { contains: location, mode: Prisma.QueryMode.insensitive } },
          { locations: { some: { city: { contains: location, mode: Prisma.QueryMode.insensitive } } } },
          { locations: { some: { state: { contains: location, mode: Prisma.QueryMode.insensitive } } } },
          { locations: { some: { zipCode: { contains: location, mode: Prisma.QueryMode.insensitive } } } },
        ],
      };
    }
