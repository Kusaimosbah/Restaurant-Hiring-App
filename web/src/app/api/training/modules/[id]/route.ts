...
export async function GET(
  ...
  try {
    ...
    if (module.prerequisites.length > 0) {
      const prerequisiteIds = module.prerequisites.map(p => p.id);
      
      // For each prerequisite, check if all its materials are completed
    ...
    const materialsWithProgress = module.materials.map(material => {
